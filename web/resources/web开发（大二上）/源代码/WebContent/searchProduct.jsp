...
<center>
    ...
    <br>
    <div>
        您的位置：<a href="index.jsp">首页</a>>>搜索结果
    </div>
    <h1>商品信息列表</h1>
    <form action="searchProduct.jsp" method="post">
        <input type='text' name="search" value="<%=search %>" placeholder="请输入想搜索的商品"/> <input
    ...
    <table border="1" width="80%">
        ...
        <%
            ...
            int proNum = prodao.getNumOfProductsLimitSearch(search);
            ...
            List<Product> products = prodao.searchProductsLimitSize(begin, pagesize, search);
            for (int i = 1; i <= products.size(); i++) {
                Product pro = products.get(i - 1);
        %>
        <tr>
            <td align='center'><%=i + begin%>
            </td>
            <td align='center'><%=pro.getName()%>
            </td>
            <td align='center'><a href="view.jsp?id=<%=pro.getId()%>">查看</a></td>
            <td align='center'><a href="addToCart.jsp?id=<%=pro.getId()%>">添加到购物车</a>
                <%
                    if (user != null && user.getRequest() != 0) {
                %> &emsp;&emsp;<a href="modifyProduct.jsp?id=<%=pro.getId()%>">修改商品信息</a>
                &emsp;&emsp; <a
            ...
            %>
            <td align='center'><%=pro.getInventory()%>
            </td>
            <td align='center'><%=pro.getPrice()%>
            </td>
        </tr>
        <%
            }
    ...
    一共找到有<%=proNum%>条关于<font color='red'>“<%=search%>”
</font>的数据，共<%=totalPage%>页。<br>
    <%
    ...
    <%
        ...
        if (products.size() == 0)
            out.print("<h1>找不到含<font color = 'red'>“" + search + "”</font>的商品</h1>");
    %>
</center>
<%
    if (user != null && user.getRequest() != 0) {
