...
public class UserServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (source.equals("register")) {//�û�ע��
            String name = request.getParameter("username");
            String psw = request.getParameter("psw1");
            UserInfoBean newUser = addUser(name, psw);//���ú���1
            request.getSession().setAttribute("user", newUser);
            request.getRequestDispatcher("/index.jsp").forward(request, response);
        }


        if (source.equals("deleteUser")) {//ɾ���û�
        ...
        if (source.equals("modifyUser")) {//�޸��û���Ϣ
            UserInfoBean userInfo = (UserInfoBean) request.getSession().getAttribute("user");
            String name = request.getParameter("username");
            String psw = request.getParameter("psw1");
            UserInfoBean newUser = modifyUser(userInfo, name, psw);//���ú���2
            request.getSession().setAttribute("user", newUser);
            request.getRequestDispatcher("/index.jsp").forward(request, response);
        }
