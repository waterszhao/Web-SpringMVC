...
<center>
    ...
    <h1>班级:<%=request.getParameter("CLname") %>
    </h1>
    <table border="1" width="80%">
        ...
        <tr>
            ...
            <td align='center'><%=student.getSname()%>
            </td>
            <td align='center'><%=student.getSsex()%>
            </td>
            <td align='center'><%=student.getSage()%>
            </td>
            <td align='center'><%=student.getCreditGot()%>
            </td>
            <td align='center'><%=student.getSposition()%>
            </td>
            <td align='center'><a
                    href='checkreport.jsp?CLno=<%=CLno %>&&Sno=<%=student.getSno() %>&&Sname=<%=student.getSname() %>'>查看课程成绩</a>
            </td>
            <td align='center'><a href='modifystudent.jsp?Sno=<%=student.getSno() %>&&Sname=<%=student.getSname() %>
				...
				&&position=<%=student.getSposition() %>&&CLno=<%=CLno%>&&CLname=<%=request.getParameter("CLname")%>'>修改学生信息</a>
            </td>
